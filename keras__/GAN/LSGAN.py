...
class LSGAN:
    def __init__(self):
        ...
        self.combined.compile(loss='mse', optimizer=Adam(2e-4))  # least square loss
    ...
    def buildGenerator(self):
        ...
        model.add(Conv2DTranspose(filters=128, kernel_size=3, strides=1, padding='same'))
        model.add(BatchNormalization(momentum=0.8))
        model.add(Activation('relu'))
    ...
    def buildDiscriminator(self):
        ...
        discriminator.compile(optimizer=Adam(2e-4), loss='mse')  # least square loss
...
def main():
    gan = LSGAN()
    gan.trainModel(epochs=10000, batch_size=64)
